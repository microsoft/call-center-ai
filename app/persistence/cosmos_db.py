...
class CosmosDbStore(IStore):
    ...
    async def call_transac(
        ...
        async def _exec() -> None:
            ...
            logger.debug("#1: %s", init_call.get("claim"))
            ...
            logger.debug("#2: %s", init_update.get("claim"))
            ...
            try:
                async with self._use_client() as db:
                    logger.debug("#3: %s", init_update.get("claim"))
                    logger.debug("#4: %s", init_call.get("claim"))
            ...
            logger.debug("#5: %s", remote_raw.get("claim"))
            ...
            logger.debug("#6: %s", remote_call.claim)
